...
# Create bar graph
library(ggplot2)
VAplot <- ggplot(traindata, aes(x=treat, y=age)) + geom_jitter(aes(color=status)) + geom_boxplot(aes(alpha = 0.3, outlier.shape = NA)) + labs(y="Age", x="Treatment") + ggtitle("Survival Status based on Treatment and Age", subtitle = "Low Number of Survivors in Both Type of Treatments")
VAplot
```